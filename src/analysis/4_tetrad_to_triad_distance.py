...
def model_get_tetrad_to_triad_distance_proportion(run_path, data, verbose=False):
    ...
    data_tetrad, data_parent_triad_1, data_parent_triad_2 = data
    
    mu_data, _  = TN_VAE.encoder(data_tetrad)
    ...
    result_str += "\tMEAN PROPORTION DISTANCE: {:.5f}".format(proportion_mu)
    result_str += "\tSTD PROPORTION DISTANCE: {:.5f}".format(proportion_std)
...
def main():
    ...
    data = (data_tetrad, data_parent_triad_1, data_parent_triad_2)
    ...
    for LATENT_DIM_PATH in LATENT_DIM_PATHS:
        ...
        for ARCHITECTURE_PATH in ARCHITECTURE_PATHS:
            ...
            for RUN_PATH in RUN_PATHS:
                ...
                result_txt += model_get_tetrad_to_triad_distance_proportion(RUN_PATH, data)



    with open(os.path.join(CODE_PATH, 'analysis_results/4_tetrad_to_triad_distance/result.txt'), 'w') as text_file:
