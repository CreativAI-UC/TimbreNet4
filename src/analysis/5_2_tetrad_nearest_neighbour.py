...
def model_nearest_neighbour_analysis(run_path, triad_chord, triad_latent, tetrad_chord, tetrad_latent, verbose=False):
    
    with open(os.path.join(run_path, 'train_params.txt')) as f:
        H_PARAMS = json.load(f)

    n_3 = np.shape(triad_latent)[0]
    n_4 = np.shape(tetrad_latent)[0]

    tree = KDTree(triad_latent)

    distances, indices = tree.query(tetrad_latent, 10)
    ...
    for i in range(n_4):
        chord_4 = tetrad_chord[i]

        nearest_0 = triad_chord[indices[i][0]]
        nearest_1 = triad_chord[indices[i][1]]
        nearest_2 = triad_chord[indices[i][2]]
        nearest_3 = triad_chord[indices[i][3]]
        nearest_4 = triad_chord[indices[i][4]]
        nearest_5 = triad_chord[indices[i][5]]
        nearest_6 = triad_chord[indices[i][6]]
        nearest_7 = triad_chord[indices[i][7]]
        nearest_8 = triad_chord[indices[i][8]]
        nearest_9 = triad_chord[indices[i][9]]
    ...
    for LATENT_DIM_PATH in LATENT_DIM_PATHS:
        ...
        for ARCHITECTURE_PATH in ARCHITECTURE_PATHS:
            ...
            for RUN_PATH in RUN_PATHS:
                ...
                tetrad_chord  = np.load(RUN_PATH+'/tetrad_chord.npy')
                tetrad_latent = np.load(RUN_PATH+'/tetrad_latent.npy')
                result_txt += model_nearest_neighbour_analysis(RUN_PATH, triad_chord, triad_latent, tetrad_chord, tetrad_latent)

    with open(os.path.join(CODE_PATH, 'analysis_results/5_nearest_neighbour/tetrad_triad_nearest.txt'), 'w') as text_file:
